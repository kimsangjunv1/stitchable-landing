...
- [ ] FAQ에 “코드 자동 반영 아님” 명시
- [ ] `data-fp-view` / `data-fp-open` / `onRevealTarget` / `onNavigate`가 SPA·모달 가이드에 포함
...
> 기존 `/guide/backend-api`의 REST 엔드포인트·DB 스키마 중심 가이드는 제거합니다. 공개 문서는 `/docs/persistence`에서 프론트엔드 `FivePixelsAdapter` 계약까지만 다룹니다.
...
1. 스테이징 URL을 연다  
2. 패널에서 **피드백 추가** 모드로 전환  
3. 요소 클릭 → 메시지 작성 → 제출  
4. 패널에서 **보기** 모드로 전환해 마커·목록 확인  
...
| 클라이언트 | 안내문 + 클릭으로 표시 |
| 모달·탭 | `data-fp-view` / `data-fp-open` 또는 `onRevealTarget` |
...
function createAdapter(): FivePixelsAdapter {
  ...
  return {
    ...
    feedback: {
      create: (payload) =>
        fetch(`${base}/feedbacks`, { method: "POST", body: JSON.stringify(payload) }).then((r) => r.json()),
      getForUi: (id) => fetch(`${base}/feedbacks/${id}/overview`).then((r) => r.json()),
      update: (id, payload) =>
        fetch(`${base}/feedbacks/${id}`, { method: "PATCH", body: JSON.stringify(payload) }).then((r) => r.json()),
    },
  };
}
...
숨김 UI 복원 순서: 선언형(`data-fp-*`) → fallback `onRevealTarget`.
...
| `ui` | appearance, locale, messages, showFeedbackList… | UI |
...
| `onRevealTarget` | `(report) => boolean \| Promise` | 숨김 타깃 노출 |
...
|------|---------------|--------|
| `idle` | 기본 상태 | 일반 화면 사용 |
| `report` | 피드백 추가 | 요소 선택·피드백 작성·UI Edit |
| `view` | 피드백 보기 | 마커·목록·답변·상태 확인 |
...
- 삭제(확인), 복사, 위치로 스크롤·펄스
- 숨김 대상 복원 (`data-fp-*`, `onRevealTarget`)
...
[1. 대상 지정]     data-report-id (± type, fp-view/open)
        ↓
[2. 피드백 작성]   패널에서 Report 선택 → 클릭 → (선택 UI Edit) → 제출
        ↓
[3. 마커 표시]     좌표·anchor 저장 (local 또는 API)
        ↓
[4. 조회·검수]     패널에서 View 선택 → 목록/마커 → 답변·멘션·상태
        ↓
...
| SPA에서 되나요? | pathname 스코프 + `onNavigate` + (필요 시) `onRevealTarget`. |
...
| 모달/오버레이 | `data-fp-*` 또는 `onRevealTarget` |
...
*이 브리프는 fivepixels 레포의 README·memo·기능 문서·타입 정의를 종합한 것입니다. 구현 세부가 충돌하면 코드(`src/shared/types/*`)와 README를 우선하세요.*
