...
function groupTextLines(element: HTMLElement) {
  ...
  if (text.includes("\n")) {
    ...
    const resolvedLineHeight = Number.isFinite(computedLineHeight)
      ? computedLineHeight
      : Math.max(...measuredLines.map((line) => line.height), 0);
...
function LineOverlay({
  ...
  return (
    ...
    >
      ...
      >
        <span
          style={{ display: "inline-block", paddingInline: horizontalInset }}
        >
          {line.text}
        </span>
      </motion.span>
...
export function Reveal({
  ...
  const scrollOffset = useMemo(
    () =>
      [
        `start ${100 - revealStartPosition}%`,
        `center ${100 - revealEndPosition}%`,
      ] as [`start ${number}%`, `center ${number}%`],
  ...
  const autoProgress = useAutoRevealProgress(
    interaction,
    scrollYProgress,
    resolvedTransition,
    delay,
  ...
  const transitionProgress = useTransitionProgress(
    rawProgress,
    interaction ? resolvedTransition : 0,
  );
  const Tag = as;
  const completeRef = useRef(false);
  const resolvedHighlightColor = highlightColor ?? midColor;
  const resolvedSoftness = revealWindow
    ? Math.max(14, revealWindow * 90)
    : softness;
