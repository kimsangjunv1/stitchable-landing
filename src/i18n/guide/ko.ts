...
export const guideKo: GuideMessages = {
  title: "시작하기",
  description:
    "Stitchable는 스테이징·QA·내부 도구 화면 위에 DOM 요소 단위 피드백을 남기는 React 라이브러리입니다. UI가 바뀌어도 마커를 복원하고, 스크린샷 없이 이슈를 검토할 수 있습니다.",
  navHome: "홈",
  codeCopy: "복사",
  codeCopied: "복사됨",
  onThisPage: "이 페이지에서",
  ...
  sections: [
    {
      ...
      blocks: [
        {
          type: "paragraph",
          text: "Stitchable은 실제 DOM 요소에 피드백을 남기고, UI 변경 후에도 마커를 복원하며, 필요 시 GitHub Issue로 승격할 수 있게 해줍니다.",
        },
        {
          type: "list",
          items: [
            "data-report-id — 화면이 바뀌어도 querySelector로 같은 요소를 찾아 마커 위치를 복원합니다.",
            "Shadow Root UI — 호스트 앱 CSS와 분리된 패널·오버레이·마커. 별도 CSS import 불필요.",
            "localStorage 또는 서버 — handler 생략 시 브라우저 저장, onList/onCreate/onUpdate로 API 연동.",
          ],
        },
      ],
    },
      ...
      blocks: [
        {
          ...
          headers: ["모드", "진입", "하는 일"],
          rows: [
            ["idle", "기본", "패널에서 Report / View / 요소 미리보기 선택"],
            ["report", "피드백 추가 · ⌘⇧M", "화면 요소를 클릭해 피드백 작성"],
            ["view", "피드백 보기 · ⌘⇧L", "마커·목록 조회, 답변·검수, Git Issue 승격"],
          ],
        },
        {
          type: "callout",
          variant: "info",
          text: "답변·검수(denied / confirm / checkout) 상세는 아래 Feedback Workflow를 참고하세요.",
        },
      ],
    },
    {
      ...
      blocks: [
        { type: "code", snippet: "install", language: "bash" },
        {
          type: "callout",
          variant: "info",
          text: "Next.js 등 SSR 환경에서는 <Report />를 클라이언트에서만 렌더링하세요 (\"use client\" + dynamic import).",
        },
      ],
    },
    {
      ...
      blocks: [
        { type: "code", snippet: "quickStart", language: "tsx" },
        {
          type: "list",
          items: [
            "project.id 생략 시 기본값 \"my-app\". stage/production·멀티 앱에서는 project={{ id }} 명시 권장.",
            "피드백을 받을 화면에 <Report />를 1회만 렌더링합니다.",
            "대상 요소에 data-report-id 필요. data-report-type 생략 시 item, 섹션은 group.",
            "CSS import 불필요 — Shadow Root 안에서 Tailwind와 함께 자동 마운트.",
          ],
        },
      ],
    },
    {
      ...
      blocks: [
        {
          type: "paragraph",
          text: "project, ui, visibility, team, fields, persistence handler, side effect, GitHub를 한 번에 연결하는 예시입니다. localStorage만 쓸 때는 handler를 생략하세요.",
        },
        {
          type: "callout",
          variant: "info",
          text: "visibility.devOnly는 production 빌드에서 UI를 숨깁니다. 스테이징 전용 QA에 적합합니다.",
        },
      ],
    },
    {
      ...
      blocks: [
        {
          type: "paragraph",
          text: "Report UI는 open Shadow Root(#stitchable-root)에 렌더링되어 호스트 앱 스타일과 격리됩니다.",
        },
    ...
    },
    {
      ...
      blocks: [
        {
          type: "paragraph",
          text: "기본 UI는 Tailwind utility class로 구성됩니다. npm 패키지 사용자는 CSS 파일을 import하지 않아도 됩니다.",
        },
        {
          type: "table",
          headers: ["목적", "경로"],
    ...
    },
    {
      ...
      blocks: [
        ...
        { type: "code", snippet: "configAdvanced", language: "tsx" },
        {
          ...
          headers: ["이름", "설명"],
          rows: [
            ["project", "{ id?, env?, version? } — id 생략 시 my-app"],
    ...
    },
    {
      ...
      blocks: [
        {
          type: "paragraph",
          text: "handler props를 넘기지 않으면 브라우저 localStorage를 사용합니다. 키는 project.id와 project.env로 분리됩니다.",
        },
        ...
        { type: "subheading", text: "Data transfer (localStorage 전용)" },
        {
          ...
          headers: ["기능", "설명"],
          rows: [
            ["Import", "JSON 파일·드래그앤드롭으로 일괄 가져오기"],
            ["Export", "현재 scope 피드백 JSON보내기"],
            ["Command", "JSON paste로 replace·merge (충돌 시 확인)"],
          ],
        },
    ...
    },
    {
      ...
      blocks: [
        {
          type: "paragraph",
          text: "피드백을 로컬에 쌓았다가 필요할 때 GitHub Issue로 승격합니다. GitHub API는 앱 서버에서 처리하는 것을 권장합니다 (github.onCreate).",
        },
        {
          type: "table",
          headers: ["필드", "설명"],
          rows: [
            ["enabled", "false면 Git Issue 버튼 숨김"],
        ...
        },
        {
          type: "callout",
          variant: "info",
          text: "전송 후 피드백 status가 git_issued로 바뀌고, 이슈 링크가 포함된 시스템 reply가 추가됩니다.",
        },
      ],
    },
    {
      ...
      blocks: [
        {
          type: "paragraph",
          text: "view 모드(⌘⇧L)에서 마커(item 빨간 점 · group 보라 점)를 기준으로 아래 흐름이 동작합니다.",
        },
        {
          type: "ordered",
          items: [
            "작성 — 요소 선택, 메시지, 작성자, checkbox 태그.",
            "마커 배지 — replies.length ≥ 1이면 +N 표시.",
            "hover — 상태 배지, 메시지, 작성자, 태그·최근 답변 미리보기.",
            "클릭 — 원본 이슈와 답변 입력 UI(태그 없음) 오픈.",
            "첫 답변 — suggested 항목 추가, denied / confirm / select 표시.",
            "denied — composer 오픈 후 found_error 답변 추가.",
            "checkout — 최근 found_error에만 표시, suggested 재답변.",
            "confirm — resolved 답변 추가, 피드백 status resolved.",
          ],
        },
        {
          type: "table",
      ...
      blocks: [
        {
          ...
          items: [
            ...
            "replies는 시간순 append, 최신 답변은 마지막 항목.",
      ...
      blocks: [
        {
          type: "list",
          items: [
            "flat projectId/environment/appVersion 대신 project={{ id, env, version }} 사용.",
            "flat prop 대신 project, ui, visibility 객체 사용.",
            "workspace 개념은 project.id로 교체.",
            "storageAdapter 제거 — onList/onCreate/onUpdate/onDelete 사용.",
            "analytics·알림은 onEvent/onReply 사용.",
