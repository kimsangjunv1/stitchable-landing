...
- `ui.messages` — `DeepPartialFivePixelsMessages`로 UI 문구를 부분 오버라이드합니다.
...
- `visibility.devOnly`를 주면 `NODE_ENV === "production"`일 때 FivePixels UI를 렌더링하지 않습니다.
- `visibility.enabled={false}`를 주면 환경과 관계없이 FivePixels UI를 렌더링하지 않습니다.

## Keyboard Shortcuts

FivePixels UI는 마우스 없이도 주요 기능을 사용할 수 있도록 키보드 단축키를 제공합니다. Mac에서는 `⌘`, Windows/Linux에서는 `Ctrl`을 modifier로 사용합니다.
