...
stitchable은 스테이징·QA·내부 도구에 DOM 요소 단위 피드백을 얹는 React 라이브러리입니다. README, package.json, 번들 측정 결과를 기준으로 장점을 정리했습니다.
...
GitHub Issue 승격 — 로컬 피드백을 나중에 Issue로 올리는 흐름(formatFeedbackAsGitHubIssueBody 포함)을 지원합니다.
통합·도입이 쉬운 점
한 줄로 시작 — <Report /> 하나 + 대상 요소에 data-report-id만 붙이면 동작합니다.
별도 CSS import 불필요 — UI가 Shadow Root(#stitchable-root)에 렌더링되며, purge된 Tailwind CSS가 번들에 포함됩니다.
...
subpath export — stitchable/report는 Report·타입만 export해, motion 등은 필요할 때만 stitchable에서 가져올 수 있습니다.
...
한 줄 요약
“React 앱에 <Report /> 하나만 넣으면, CSS 충돌 없이 DOM 단위 QA·피드백·검수·GitHub 연동까지 돌아가고, 서버 없이도 localStorage로 바로 쓸 수 있는 라이브러리”

Marker.io, BugHerd, Usersnap, Jam.dev 같은 SaaS QA 위젯과 stitchable을 나란히 보면, 포지션이 꽤 다릅니다. SaaS는 “바로 쓰는 올인원 서비스”, stitchable은 “내 React 앱에 박아 넣는 오픈소스 라이브러리”에 가깝습니다.

한눈에 보는 포지셔닝
구분 Marker.io / BugHerd / Usersnap / Jam stitchable
형태
SaaS + 외부 위젯/확장 프로그램
npm 라이브러리 (앱 안에 내장)
비용
...
내부 QA·스테이징·개발팀
stitchable이 SaaS QA 도구 대비 유리한 점

1. 비용·벤더 락인 없음
   Marker.io는 월 $39부터, BugHerd는 멤버 기준 월 구독입니다.
   stitchable은 MIT + 런타임 dependency 0이라 시트 수·프로젝트 수·기간 제한이 없습니다.
   SaaS를 끊어도 피드백 UI·데이터 흐름을 내 코드/인프라로 유지할 수 있습니다.
2. 데이터·프라이버시를 완전히 내가 소유
   Marker.io/Jam 등은 피드백이 외부 서비스로 올라갑니다.
   stitchable은 기본이 localStorage, 서버 연동 시에도 onList/onCreate/onUpdate로 내 API·내 DB에만 저장합니다.
...
3. “요소 단위” 피드백 — 스크린샷보다 개발 친화적
   ...
   stitchable은 data-report-id로 실제 DOM 요소에 앵커를 겁니다. 화면이 바뀌어도 querySelector로 같은 요소를 찾아 마커를 복원합니다.
...
4. 앱에 녹아든 QA 도구 (외부 위젯이 아님)
   ...
   stitchable은 <Report /> 하나로 React 트리에 들어가고, UI는 Shadow Root에 격리됩니다.
   호스트 CSS와 충돌 없고, 별도 CSS import도 필요 없습니다.
5. production에 안 보이게 설계됨
   visibility.devOnly로 production 빌드에서는 UI를 숨길 수 있습니다.
   Marker.io 등은 스테이징/프로덕션 환경 분리는 되지만, “앱 코드 레벨에서 아예 렌더 안 함”은 stitchable이 더 직접적입니다.
   내부 QA 전용 도구로 쓰기 좋습니다.
6. 검수 워크플로우가 라이브러리에 내장
   SaaS 대부분: 코멘트 → 이슈 트래커로 넘기기.
   stitchable: suggested → denied / confirm / checkout 검수 타임라인이 view 모드에 포함됩니다.
   Jira까지 안 가도, 스테이징에서 PM·디자이너·개발자가 한 화면에서 검수할 수 있습니다.
7. 저장소·연동을 내 방식으로
   SaaS: Jira/Linear/GitHub 제공 방식에 맞춤 (플랜·동기화 방식 제한).
   stitchable: persistence handler로 어떤 백엔드든 연결 가능하고, GitHub Issue도 github.onCreate로 내 서버 API 경유.
...
솔직히 SaaS가 더 나은 경우 (stitchable 약점)
비교를 공정하게 하려면 이쪽도 짚어야 합니다.

SaaS가 유리한 점 stitchable 현재
...
스테이징에서 개발팀이 DOM 단위로 QA·검수하고, 데이터는 내가 갖고, 구독은 안 쓰고 싶다 → stitchable
요약
stitchable의 차별점은 “더 많은 QA 기능”이 아니라 소유권·비용·DOM 앵커·React 내장·검수 워크플로우 쪽입니다. Marker.io류는 올인원 SaaS + 시각적 리포팅 + PM 연동이 강하고, stitchable은 내 앱 안의 가벼운 오픈소스 QA 레이어에 가깝습니다.
